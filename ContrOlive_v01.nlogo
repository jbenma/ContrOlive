...
to Clear
  ...
  set-default-shape olives "olive"
  ...
  set-default-shape flies "fly"
  create-flies initial-number-flies
  [
    set color white
    set size 2
    set energy random 300
    setxy random-xcor random-ycor
  ]
...
to Go
 if not any? flies [stop]
 ask flies [move fed reproduce tire death]
 ask spiders [eat-fly]
 display-energy
 tick
 end
...
to move
  ask flies [right random 20 left random 20 forward 1]
end

to fed
  ask flies [if any? flowers-on patch-ahead 1[set energy energy + 15]]
end
  
to reproduce
  ask flies [if any? olives-on patch-ahead 1[hatch-flies 1 [right random 20 left random 20 forward 2]]]
end

to tire
  ask flies [set energy energy - 1]
end
...
to eat-fly
  ask flies [if any? spiders-on patch-ahead 1 [die]]
end

to display-energy
  if Display-energy? [ask flies[set label round energy]]
end
@#$#@#$#@
GRAPHICS-WINDOW
210
